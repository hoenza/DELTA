...
from DELTA.Engine.SnapKV.model import Attention, FeedForward, Transformer
...
def init_dist(draft_ranks=None):
    ...
    init_kwargs = {}
    init_method = os.environ.get("DELTA_DIST_INIT_METHOD")
    if init_method:
...
def _apply_tp_linear(linear: nn.Linear, style: str, weight_splits: List[int] = [], rank_group=None, num_kv_heads = None, num_heads = None, head_dim = None) -> None:
    ...
    if linear.bias is not None:
        if weight_splits:  # fused QKV (bias is [Q | K | V])
            bq, bk, bv = linear.bias.split(weight_splits, dim=0)
            # q indices are in channel space, kv indices are head_dim space
            bq = bq[q_start:q_end]
            bk = bk[kv_start:kv_end]
            bv = bv[kv_start:kv_end]
            sharded_bias = torch.cat((bq, bk, bv), dim=0)
        elif style == "colwise":
            # generic col-parallel linear: split bias same as out_features
            global_rank = _get_global_rank()
            rank = rank_group.index(global_rank)
            world_size = len(rank_group)
            sharded_bias = torch.chunk(linear.bias, world_size, dim=0)[rank]
        else:
            # row-parallel: out_features unchanged, keep full bias
            sharded_bias = linear.bias
...
def _apply_tp_attn(attn: Attention, rank_group, config, group) -> None:
    ...
    # keep runtime aliases in sync with TP-local counts
    attn.n_q_heads  = attn.n_head
    attn.n_kv_heads = attn.n_local_heads
