...
def update_kv(
        ...
        # Try using FlashInfer's helper function
        # page_size = 1  # Your page size
        page_size = 16  # Your page size
        current_seq_lens = flashinfer.get_seq_lens(kv_page_indptr, kv_page_last_len, page_size)
        
        batch_indices, positions = flashinfer.get_batch_indices_positions(
            kv_append_indptr,
            current_seq_lens,
            k.shape[0]  # total tokens
        )
        
        flashinfer.append_paged_kv_cache(
            k,                    # append_key
            v,                    # append_value
            batch_indices,        # batch_indices from helper
            positions,            # positions from helper
            kv_cache,            # paged_kv_cache
            kv_page_indices,     # kv_indices
            kv_page_indptr,      # kv_indptr
            kv_page_last_len,    # kv_last_page_len
        )
...
def load_model_DELTA(checkpoint_path, device, precision, use_tp, rank_group=None, group=None):
    ...
    if "int8" in str(checkpoint_path):
        ...
        from DELTA.Engine.quantize import WeightOnlyInt8QuantHandler
    ...
    if use_tp:
        from DELTA.Engine.tp import apply_tp
...
def load_model_snapKV(checkpoint_path, device, precision, use_tp, rank_group=None, group=None):
    from DELTA.Engine.SnapKV.model import Transformer
    with torch.device('meta'):
        model = Transformer.from_name(checkpoint_path.parent.name)
...
def load_model_draft_snapKV(checkpoint_path, device, precision, use_tp, rank_group=None, group=None):
    import DELTA.Engine.SnapKV.model_draft as draft
    with torch.device('meta'):
...
def load_model_streamingLLM(checkpoint_path, device, precision, use_tp, rank_group=None, group=None):
    from DELTA.Engine.StreamingLLM.model import Transformer
    with torch.device('meta'):
        model = Transformer.from_name(checkpoint_path.parent.name)
...
def load_model_draft_streamingLLM(checkpoint_path, device, precision, use_tp, rank_group=None, group=None):
    import DELTA.Engine.StreamingLLM.model_draft as draft
    with torch.device('meta'):
        model = draft.Transformer.from_name(checkpoint_path.parent.name)
    checkpoint = torch.load(str(checkpoint_path), mmap=True, weights_only=True)
    if "model" in checkpoint and "stories" in str(checkpoint_path):
